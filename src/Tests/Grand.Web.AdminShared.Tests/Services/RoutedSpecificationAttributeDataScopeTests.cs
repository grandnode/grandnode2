...
/// <summary>
///     Covers the routing decision itself - which concrete IAdminDataScope&lt;SpecificationAttribute&gt; the resolver
...
/// </summary>
[TestClass]
public class RoutedSpecificationAttributeDataScopeTests
{
    private const string StaffStoreId = "store-1";

    private GlobalAdminDataScope<SpecificationAttribute> _globalScope = null!;
    private StoreAdminDataScope<SpecificationAttribute> _storeScope = null!;
    ...
    public void Setup()
    {
        ...
        _globalScope = new GlobalAdminDataScope<SpecificationAttribute>();
        _storeScope = new StoreAdminDataScope<SpecificationAttribute>(contextAccessor.Object);
    }

    private RoutedSpecificationAttributeDataScope ResolverForArea(string? area)
    {
        var httpContext = new DefaultHttpContext();
        if (area is not null) httpContext.Request.RouteValues["area"] = area;
        var httpContextAccessor = new Mock<IHttpContextAccessor>();
        httpContextAccessor.Setup(x => x.HttpContext).Returns(httpContext);
        return new RoutedSpecificationAttributeDataScope(httpContextAccessor.Object, _globalScope, _storeScope);
    }
    ...
    public void NoHttpContext_Throws()
    {
        ...
        var resolver = new RoutedSpecificationAttributeDataScope(httpContextAccessor.Object, _globalScope, _storeScope);
        Assert.Throws<InvalidOperationException>(() => _ = resolver.ResourceKeyPrefix);
    }
}
