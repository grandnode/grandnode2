...
[TestClass]
public class StoreDeliveryDateDataScopeTests
{
    private static StoreDeliveryDateDataScope Build(string staffStoreId)
    {
        ...
        return new StoreDeliveryDateDataScope(contextAccessorMock.Object);
    }
    ...
    public async Task HasAccess_MatchingStoreId_True()
    {
        ...
        Assert.IsTrue(await scope.HasAccess(new DeliveryDate { StoreId = "store-1" }));
    }
    ...
    {
        var scope = Build("store-1");
        Assert.IsFalse(await scope.HasAccess(new DeliveryDate { StoreId = "store-2" }));
    }

    [TestMethod]
    public async Task HasAccess_GlobalDeliveryDate_False()
    {
        var scope = Build("store-1");
        Assert.IsFalse(await scope.HasAccess(new DeliveryDate { StoreId = "" }));
    }
