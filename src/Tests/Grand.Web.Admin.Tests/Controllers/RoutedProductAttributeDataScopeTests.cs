...
/// <summary>
///     Covers the routing decision itself - which concrete IAdminDataScope&lt;ProductAttribute&gt; the resolver
...
/// </summary>
[TestClass]
public class RoutedProductAttributeDataScopeTests
{
    private const string StaffStoreId = "store-1";

    private GlobalAdminDataScope<ProductAttribute> _globalScope = null!;
    private StoreAdminDataScope<ProductAttribute> _storeScope = null!;
    ...
    public void Setup()
    {
        ...
        _globalScope = new GlobalAdminDataScope<ProductAttribute>();
        _storeScope = new StoreAdminDataScope<ProductAttribute>(contextAccessor.Object);
    }

    private RoutedProductAttributeDataScope ResolverForArea(string? area)
    {
        var httpContext = new DefaultHttpContext();
        if (area is not null) httpContext.Request.RouteValues["area"] = area;
        var httpContextAccessor = new Mock<IHttpContextAccessor>();
        httpContextAccessor.Setup(x => x.HttpContext).Returns(httpContext);
        return new RoutedProductAttributeDataScope(httpContextAccessor.Object, _globalScope, _storeScope);
    }
    ...
    public void NoHttpContext_Throws()
    {
        ...
        var resolver = new RoutedProductAttributeDataScope(httpContextAccessor.Object, _globalScope, _storeScope);
        Assert.Throws<InvalidOperationException>(() => _ = resolver.ResourceKeyPrefix);
    }
}
