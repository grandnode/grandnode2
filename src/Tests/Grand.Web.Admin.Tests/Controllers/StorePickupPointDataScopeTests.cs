...
[TestClass]
public class StorePickupPointDataScopeTests
{
    private static StorePickupPointDataScope Build(string staffStoreId)
    {
        ...
        return new StorePickupPointDataScope(contextAccessorMock.Object);
    }
    ...
    public async Task HasAccess_MatchingStoreId_True()
    {
        ...
        Assert.IsTrue(await scope.HasAccess(new PickupPoint { StoreId = "store-1" }));
    }
    ...
    {
        var scope = Build("store-1");
        Assert.IsFalse(await scope.HasAccess(new PickupPoint { StoreId = "store-2" }));
    }

    [TestMethod]
    public async Task HasAccess_GlobalPickupPoint_False()
    {
        var scope = Build("store-1");
        Assert.IsFalse(await scope.HasAccess(new PickupPoint { StoreId = "" }));
    }
