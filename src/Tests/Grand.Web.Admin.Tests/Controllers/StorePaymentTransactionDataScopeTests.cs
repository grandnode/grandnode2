...
{
    private static StorePaymentTransactionDataScope Build(string staffStoreId)
    {
        ...
        return new StorePaymentTransactionDataScope(contextAccessorMock.Object);
    }
    ...
    public async Task HasAccess_MatchingStoreId_True()
    {
        ...
        Assert.IsTrue(await scope.HasAccess(new PaymentTransaction { StoreId = "store-1" }));
    }
    ...
    {
        var scope = Build("store-1");
        Assert.IsFalse(await scope.HasAccess(new PaymentTransaction { StoreId = "store-2" }));
    }
