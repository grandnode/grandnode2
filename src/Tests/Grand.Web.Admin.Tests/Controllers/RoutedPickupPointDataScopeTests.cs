...
[TestClass]
public class RoutedPickupPointDataScopeTests
{
    private static RoutedPickupPointDataScope Build(string area)
    {
        ...
        var global = new GlobalAdminDataScope<PickupPoint>();
        var store = new StorePickupPointDataScope(BuildContextAccessor());
        return new RoutedPickupPointDataScope(httpContextAccessorMock.Object, global, store);
    }
    ...
    public void CanView_StoreArea_ForwardsToStoreScope()
    {
        ...
        Assert.IsFalse(routed.CanView(new PickupPoint { StoreId = "store-2" }).Result);
    }
