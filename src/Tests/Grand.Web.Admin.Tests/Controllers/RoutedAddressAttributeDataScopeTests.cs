...
/// <summary>
///     Covers the routing decision itself - which concrete IAdminDataScope&lt;AddressAttribute&gt; the resolver
...
/// </summary>
[TestClass]
public class RoutedAddressAttributeDataScopeTests
{
    private const string StaffStoreId = "store-1";

    private GlobalAdminDataScope<AddressAttribute> _globalScope = null!;
    private StoreAdminDataScope<AddressAttribute> _storeScope = null!;
    ...
    public void Setup()
    {
        ...
        _globalScope = new GlobalAdminDataScope<AddressAttribute>();
        _storeScope = new StoreAdminDataScope<AddressAttribute>(contextAccessor.Object);
    }

    private RoutedAddressAttributeDataScope ResolverForArea(string? area)
    {
        var httpContext = new DefaultHttpContext();
        if (area is not null) httpContext.Request.RouteValues["area"] = area;
        var httpContextAccessor = new Mock<IHttpContextAccessor>();
        httpContextAccessor.Setup(x => x.HttpContext).Returns(httpContext);
        return new RoutedAddressAttributeDataScope(httpContextAccessor.Object, _globalScope, _storeScope);
    }
    ...
    public void NoHttpContext_Throws()
    {
        ...
        var resolver = new RoutedAddressAttributeDataScope(httpContextAccessor.Object, _globalScope, _storeScope);
        Assert.Throws<InvalidOperationException>(() => _ = resolver.ResourceKeyPrefix);
    }
}
