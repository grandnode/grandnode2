...
[TestClass]
public class RoutedDeliveryDateDataScopeTests
{
    private static RoutedDeliveryDateDataScope Build(string area)
    {
        ...
        var global = new GlobalAdminDataScope<DeliveryDate>();
        var store = new StoreDeliveryDateDataScope(BuildContextAccessor());
        return new RoutedDeliveryDateDataScope(httpContextAccessorMock.Object, global, store);
    }
    ...
    public void CanView_StoreArea_ForwardsToStoreScope()
    {
        ...
        Assert.IsFalse(routed.CanView(new DeliveryDate { StoreId = "store-2" }).Result);
    }
