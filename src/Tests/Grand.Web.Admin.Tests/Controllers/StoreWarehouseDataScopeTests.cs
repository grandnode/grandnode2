...
[TestClass]
public class StoreWarehouseDataScopeTests
{
    private static StoreWarehouseDataScope Build(string staffStoreId)
    {
        ...
        return new StoreWarehouseDataScope(contextAccessorMock.Object);
    }
    ...
    public async Task HasAccess_MatchingStoreId_True()
    {
        ...
        Assert.IsTrue(await scope.HasAccess(new Warehouse { StoreId = "store-1" }));
    }
    ...
    {
        var scope = Build("store-1");
        Assert.IsFalse(await scope.HasAccess(new Warehouse { StoreId = "store-2" }));
    }

    [TestMethod]
    public async Task HasAccess_GlobalWarehouse_False()
    {
        var scope = Build("store-1");
        Assert.IsFalse(await scope.HasAccess(new Warehouse { StoreId = "" }));
    }
