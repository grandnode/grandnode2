...
/// <summary>
///     Covers the routing decision itself - which concrete IAdminDataScope&lt;CheckoutAttribute&gt; the resolver
...
/// </summary>
[TestClass]
public class RoutedCheckoutAttributeDataScopeTests
{
    private const string StaffStoreId = "store-1";

    private GlobalAdminDataScope<CheckoutAttribute> _globalScope = null!;
    private StoreAdminDataScope<CheckoutAttribute> _storeScope = null!;
    ...
    public void Setup()
    {
        ...
        _globalScope = new GlobalAdminDataScope<CheckoutAttribute>();
        _storeScope = new StoreAdminDataScope<CheckoutAttribute>(contextAccessor.Object);
    }

    private RoutedCheckoutAttributeDataScope ResolverForArea(string? area)
    {
        var httpContext = new DefaultHttpContext();
        if (area is not null) httpContext.Request.RouteValues["area"] = area;
        var httpContextAccessor = new Mock<IHttpContextAccessor>();
        httpContextAccessor.Setup(x => x.HttpContext).Returns(httpContext);
        return new RoutedCheckoutAttributeDataScope(httpContextAccessor.Object, _globalScope, _storeScope);
    }
    ...
    public void NoHttpContext_Throws()
    {
        ...
        var resolver = new RoutedCheckoutAttributeDataScope(httpContextAccessor.Object, _globalScope, _storeScope);
        Assert.Throws<InvalidOperationException>(() => _ = resolver.ResourceKeyPrefix);
    }
}
