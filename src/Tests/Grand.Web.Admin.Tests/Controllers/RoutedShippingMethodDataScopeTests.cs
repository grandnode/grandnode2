...
[TestClass]
public class RoutedShippingMethodDataScopeTests
{
    private static RoutedShippingMethodDataScope Build(string area)
    {
        ...
        var global = new GlobalAdminDataScope<ShippingMethod>();
        var store = new StoreShippingMethodDataScope(BuildContextAccessor());
        return new RoutedShippingMethodDataScope(httpContextAccessorMock.Object, global, store);
    }
    ...
    public void CanView_StoreArea_ForwardsToStoreScope()
    {
        ...
        Assert.IsFalse(routed.CanView(new ShippingMethod { StoreId = "store-2" }).Result);
    }
