...
// [AuthorizeAdmin]/[AutoValidateAntiforgeryToken]/[AuthorizeMenu] entirely, because BaseProductController
// can't inherit any single host's base controller and so those attributes no longer arrive
// transitively. Following the plan literally would have shipped Admin's product management with no
...
public class ProductControllerAttributesTests
{
    ...
    public void ProductController_CarriesRequiredAuthorizationAndCsrfAttributes()
    {
        ...
        Assert.IsTrue(type.IsDefined(typeof(AuthorizeAdminAttribute), true), "Missing [AuthorizeAdmin].");
