...
namespace Grand.Business.Checkout.Events.Shipping
{
    ...
    public class ShoppingCartItemDeletedEventHandler : INotificationHandler<EntityDeleted<ShoppingCartItem>>
    {
        ...
        {
            if ((notification.Entity.RentalStartDateUtc.HasValue && notification.Entity.RentalEndDateUtc.HasValue) || !string.IsNullOrEmpty(notification.Entity.ReservationId))
            {
                var reserved = await _productReservationService.GetCustomerReservationsHelperBySciId(notification.Entity.Id);
