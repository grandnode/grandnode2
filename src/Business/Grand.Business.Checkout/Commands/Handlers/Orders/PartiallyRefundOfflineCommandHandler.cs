...
public class PartiallyRefundOfflineCommandHandler : IRequestHandler<PartiallyRefundOfflineCommand, bool>
{
    ...
    public async Task<bool> Handle(PartiallyRefundOfflineCommand command, CancellationToken cancellationToken)
    {
        ...
        //notifications
        _ = await _messageProviderService.SendOrderRefundedStoreOwnerMessage(order, amountToRefund,
            _languageSettings.DefaultAdminLanguageId);

        _ = await _messageProviderService.SendOrderRefundedCustomerMessage(order, amountToRefund,
