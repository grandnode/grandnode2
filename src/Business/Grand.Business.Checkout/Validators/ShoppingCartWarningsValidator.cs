...
namespace Grand.Business.Checkout.Validators
{
    ...
    public record ShoppingCartWarningsValidatorRecord(Customer Customer, Store Store, IList<ShoppingCartItem> ShoppingCarts);
        ...
        public ShoppingCartWarningsValidator(ITranslationService translationService, IProductService productService)
        {
            ...
            RuleFor(x => x).CustomAsync(async (value, context, ct) =>
            {
                ...
                foreach (var sci in value.ShoppingCarts)
                {
                    ...
                    if (product == null)
                    {
                        context.AddFailure(string.Format(translationService.GetResource("ShoppingCart.CannotLoadProduct"), sci.ProductId));
                        return;
                    }
