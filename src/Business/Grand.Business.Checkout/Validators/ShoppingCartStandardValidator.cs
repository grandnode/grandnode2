...
namespace Grand.Business.Checkout.Validators
{
    ...
    public record ShoppingCartStandardValidatorRecord(Customer Customer, Product Product, ShoppingCartItem ShoppingCartItem);
    ...
    public class ShoppingCartStandardValidator : AbstractValidator<ShoppingCartStandardValidatorRecord>
    {
        public ShoppingCartStandardValidator(ITranslationService translationService, IAclService aclService)
        {
            RuleFor(x => x).Custom((value, context) =>
            {
                ...
                //call for price
                if (value.ShoppingCartItem.ShoppingCartTypeId == ShoppingCartType.ShoppingCart && value.Product.CallForPrice)
                {
                    context.AddFailure(translationService.GetResource("Products.CallForPrice"));
                }
                ...
                if (value.Product.EnteredPrice)
                {
                    ...
                    {
                        context.AddFailure(string.Format(translationService.GetResource("ShoppingCart.CustomerEnteredPrice.RangeError"),
                ...
                if (value.Product.AvailableStartDateTimeUtc.HasValue)
                {
                    ...
                    DateTime availableStartDateTime = DateTime.SpecifyKind(value.Product.AvailableStartDateTimeUtc.Value, DateTimeKind.Utc);
