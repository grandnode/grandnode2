...
public class CheckoutAttributeController : BaseStoreController
{
    #region Constructors
    ...
    /// <summary>
    /// Checks access permissions for a checkout attribute and handles warnings
    /// </summary>
    /// <param name="checkoutAttribute">Checkout attribute to check</param>
    /// <returns>True if access is allowed, false if access should be denied</returns>
    private bool CheckAccessPermission(CheckoutAttribute checkoutAttribute)
    {
        if (checkoutAttribute == null)
            return false;

        if (!checkoutAttribute.LimitedToStores || (checkoutAttribute.LimitedToStores &&
                checkoutAttribute.Stores.Contains(_contextAccessor.WorkContext.CurrentCustomer.StaffStoreId) &&
                checkoutAttribute.Stores.Count > 1))
        {
            Warning(_translationService.GetResource("admin.Catalog.attributes.checkoutAttributes.permissions"));
            return true;
        }

        return checkoutAttribute.AccessToEntityByStore(_contextAccessor.WorkContext.CurrentCustomer.StaffStoreId);
    }
    ...
    {
        var checkoutAttributes = await _checkoutAttributeViewModelService.PrepareCheckoutAttributeListModel(_contextAccessor.WorkContext.CurrentCustomer.StoreId);
    ...
    public async Task<IActionResult> Edit(string id)
    {
        ...
        if (!CheckAccessPermission(checkoutAttribute))
            return RedirectToAction("List");
    ...
    {
        var ca = await _checkoutAttributeService.GetCheckoutAttributeById(checkoutAttributeId);
        if (!CheckAccessPermission(ca))
            return View("AccessDenied", _translationService.GetResource("admin.Catalog.attributes.checkoutAttributes.permissions"));
    ...
    public async Task<IActionResult> ValueCreatePopup(string checkoutAttributeId)
    {
        ...
        if (!ca.AccessToEntityByStore(_contextAccessor.WorkContext.CurrentCustomer.StaffStoreId))
            return View("AccessDenied", _translationService.GetResource("admin.Catalog.attributes.checkoutattributes.permissions"));
