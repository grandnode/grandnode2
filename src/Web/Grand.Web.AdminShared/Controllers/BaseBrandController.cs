    ...
    : BaseController
{
        ...
        [FromServices] IImportManager<BrandDto> importManager)
    {
        try
        {
            // ImportManager<T>/BrandImportDataObject match rows by an attacker-supplied Id and
            // upsert with no ownership check at all (see project security-fix history) - a
            // store-scoped caller could otherwise create or silently overwrite ANY store's brand.
