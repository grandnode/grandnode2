    ...
    : BaseController
{
    ...
    public async Task<IActionResult> ExportXlsx([FromServices] IExportManager<Collection> exportManager)
    {
        try
        {
            var bytes = await exportManager.Export(await collectionService.GetAllCollections(collectionName: "", storeId: scope.DefaultStoreId ?? "", showHidden: true));
        ...
        [FromServices] IImportManager<CollectionDto> importManager)
    {
        try
        {
            // ImportManager<T>/CollectionImportDataObject match rows by an attacker-supplied Id and
            // upsert with no ownership check at all (see project security-fix history) - a
            // store-scoped caller could otherwise create or silently overwrite ANY store's
            // collection. Deny outright for non-global scope rather than threading scope through the
            // shared, multi-entity import engine.
            ...
            Success(translationService.GetResource("Admin.Catalog.Collection.Imported"));
            return RedirectToAction("List");
        }
        catch (Exception exc)
        {
