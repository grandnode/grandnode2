...
/// <summary>
///     Resolves the correct per-host <see cref="IAdminDataScope{BlogPost}" /> implementation at
...
///     AddScoped&lt;IAdminDataScope&lt;BlogPost&gt;, X&gt;() per host would silently let whichever
...
/// </summary>
public class RoutedBlogPostDataScope(
    IHttpContextAccessor httpContextAccessor,
    GlobalAdminDataScope<BlogPost> globalScope,
    StoreAdminDataScope<BlogPost> storeScope) : IAdminDataScope<BlogPost>
{
    private IAdminDataScope<BlogPost> Resolved
    {
        get
        {
            ...
            return area switch {
                ...
                //missing area (including "Vendor" - BlogPost has no Vendor screen) must never
                //silently resolve to the unscoped global scope
                _ => throw new InvalidOperationException(
                    $"RoutedBlogPostDataScope: unrecognized or missing area '{area}'.")
            };
        }
    }

    public Task<bool> HasAccess(BlogPost entity) => Resolved.HasAccess(entity);

    public Task<bool> CanView(BlogPost entity) => Resolved.CanView(entity);
