...
/// <summary>
///     Resolves the correct per-host <see cref="IAdminDataScope{NewsItem}" /> implementation at
...
///     AddScoped&lt;IAdminDataScope&lt;NewsItem&gt;, X&gt;() per host would silently let whichever
///     host's StartupApplication ran last win for every area in that process.
///
///     No Vendor branch: Vendor has no News screen at all, so any "Vendor" (or other
///     unrecognized/missing) area value fails closed.
/// </summary>
public class RoutedNewsItemDataScope(
    IHttpContextAccessor httpContextAccessor,
    GlobalAdminDataScope<NewsItem> globalScope,
    StoreAdminDataScope<NewsItem> storeScope) : IAdminDataScope<NewsItem>
{
    private IAdminDataScope<NewsItem> Resolved
    {
        get
        {
            ...
            return area switch {
                ...
                //missing area (including "Vendor" - NewsItem has no Vendor screen) must never
                //silently resolve to the unscoped global scope
                _ => throw new InvalidOperationException(
                    $"RoutedNewsItemDataScope: unrecognized or missing area '{area}'.")
            };
        }
    }

    public Task<bool> HasAccess(NewsItem entity) => Resolved.HasAccess(entity);

    public Task<bool> CanView(NewsItem entity) => Resolved.CanView(entity);
