...
/// <summary>
///     Resolves the correct per-host <see cref="IAdminDataScope{CheckoutAttribute}" /> implementation
...
///
///     No Vendor branch: CheckoutAttribute has no Vendor screen at all, so any "Vendor" (or other
///     unrecognized/missing) area value fails closed.
/// </summary>
public class RoutedCheckoutAttributeDataScope(
    IHttpContextAccessor httpContextAccessor,
    GlobalAdminDataScope<CheckoutAttribute> globalScope,
    StoreAdminDataScope<CheckoutAttribute> storeScope) : IAdminDataScope<CheckoutAttribute>
{
    private IAdminDataScope<CheckoutAttribute> Resolved
    {
        get
        {
            ...
            return area switch {
                ...
                _ => throw new InvalidOperationException(
                    $"RoutedCheckoutAttributeDataScope: unrecognized or missing area '{area}'.")
            };
        }
    }

    public Task<bool> HasAccess(CheckoutAttribute entity) => Resolved.HasAccess(entity);
    public Task<bool> CanView(CheckoutAttribute entity) => Resolved.CanView(entity);
