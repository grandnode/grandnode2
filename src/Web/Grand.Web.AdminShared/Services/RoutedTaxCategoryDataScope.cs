...
/// <summary>
///     Resolves the correct per-host <see cref="IAdminDataScope{TaxCategory}" /> implementation
///     at request time, based on the current request's "area" route value — same fix and same
///     reason as <see cref="RoutedEmailAccountDataScope"/> (see that file's doc comment):
///     Grand.Web (the combined host) loads Admin and Store together in one DI container, so a
///     plain AddScoped&lt;IAdminDataScope&lt;TaxCategory&gt;, X&gt;() per host would silently
///     let whichever host's StartupApplication ran last win for every area in that process.
///
///     There is no Vendor branch: Vendor has no TaxController at all, so any "Vendor" (or other
///     unrecognized/missing) area value fails closed.
/// </summary>
public class RoutedTaxCategoryDataScope(
    IHttpContextAccessor httpContextAccessor,
    GlobalAdminDataScope<TaxCategory> globalScope,
    StoreTaxCategoryDataScope storeScope) : IAdminDataScope<TaxCategory>
{
    private IAdminDataScope<TaxCategory> Resolved
    {
        get
        {
            ...
            return area switch {
                ...
                //missing area (including "Vendor" - TaxCategory has no Vendor screen) must
                //never silently resolve to the unscoped global scope
                _ => throw new InvalidOperationException(
                    $"RoutedTaxCategoryDataScope: unrecognized or missing area '{area}'.")
            };
        }
    }

    public Task<bool> HasAccess(TaxCategory entity) => Resolved.HasAccess(entity);
