...
/// <summary>
///     Resolves the correct per-host <see cref="IAdminDataScope{SpecificationAttribute}" /> implementation
...
///
///     No Vendor branch: SpecificationAttribute has no Vendor screen at all, so any "Vendor" (or other
///     unrecognized/missing) area value fails closed.
/// </summary>
public class RoutedSpecificationAttributeDataScope(
    IHttpContextAccessor httpContextAccessor,
    GlobalAdminDataScope<SpecificationAttribute> globalScope,
    StoreAdminDataScope<SpecificationAttribute> storeScope) : IAdminDataScope<SpecificationAttribute>
{
    private IAdminDataScope<SpecificationAttribute> Resolved
    {
        get
        {
            ...
            return area switch {
                ...
                _ => throw new InvalidOperationException(
                    $"RoutedSpecificationAttributeDataScope: unrecognized or missing area '{area}'.")
            };
        }
    }

    public Task<bool> HasAccess(SpecificationAttribute entity) => Resolved.HasAccess(entity);
    public Task<bool> CanView(SpecificationAttribute entity) => Resolved.CanView(entity);
