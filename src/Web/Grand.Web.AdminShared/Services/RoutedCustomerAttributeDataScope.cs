...
/// <summary>
///     Resolves the correct per-host <see cref="IAdminDataScope{CustomerAttribute}" /> implementation
...
///
///     No Vendor branch: CustomerAttribute has no Vendor screen at all, so any "Vendor" (or other
///     unrecognized/missing) area value fails closed.
/// </summary>
public class RoutedCustomerAttributeDataScope(
    IHttpContextAccessor httpContextAccessor,
    GlobalAdminDataScope<CustomerAttribute> globalScope,
    StoreAdminDataScope<CustomerAttribute> storeScope) : IAdminDataScope<CustomerAttribute>
{
    private IAdminDataScope<CustomerAttribute> Resolved
    {
        get
        {
            ...
            return area switch {
                ...
                _ => throw new InvalidOperationException(
                    $"RoutedCustomerAttributeDataScope: unrecognized or missing area '{area}'.")
            };
        }
    }

    public Task<bool> HasAccess(CustomerAttribute entity) => Resolved.HasAccess(entity);
    public Task<bool> CanView(CustomerAttribute entity) => Resolved.CanView(entity);
