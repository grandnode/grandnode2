...
namespace Grand.Web.Validators.Customer
{
    public class LoginValidator : BaseGrandValidator<LoginModel>
    {
            ...
            : base(validators)
        {
            ...
            {
                var customer = customerSettings.UsernamesEnabled ? await customerService.GetCustomerByUsername(x.Username) : await customerService.GetCustomerByEmail(x.Email);

                switch (customer)
                {
                    case null:
                        context.AddFailure(translationService.GetResource("Account.Login.WrongCredentials.CustomerNotExist"));
                        break;
                    case { Deleted: true }:
