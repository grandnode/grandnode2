...
namespace Grand.Web.Validators.Catalog
{
    ...
    public class ProductReviewsValidator : BaseGrandValidator<ProductReviewsModel>
    {
        public ProductReviewsValidator(
            ...
            : base(validators)
        {
            RuleFor(x => x.AddProductReview.Title).NotEmpty().WithMessage(translationService.GetResource("Reviews.Fields.Title.Required"));
            RuleFor(x => x.AddProductReview.Title).Length(1, 200).WithMessage(string.Format(translationService.GetResource("Reviews.Fields.Title.MaxLengthValidation"), 200));
            RuleFor(x => x.AddProductReview.ReviewText).NotEmpty().WithMessage(translationService.GetResource("Reviews.Fields.ReviewText.Required"));
            ...
            RuleFor(x => x).CustomAsync(async (x, context, _) =>
            {
                ...
                if (await groupService.IsGuest(workContext.CurrentCustomer) && !catalogSettings.AllowAnonymousUsersToReviewProduct)
                {
                ...
                if (catalogSettings.ProductReviewPossibleOnlyAfterPurchasing &&
                    !(await orderService.SearchOrders(customerId: workContext.CurrentCustomer.Id, productId: x.ProductId, os: (int)OrderStatusSystem.Complete)).Any())
                ...
                {
                    var reviews = await productReviewService.GetAllProductReviews(customerId: workContext.CurrentCustomer.Id,
