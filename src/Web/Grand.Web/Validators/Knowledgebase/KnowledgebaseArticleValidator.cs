...
namespace Grand.Web.Validators.Knowledgebase
{
    public class KnowledgebaseArticleValidator : BaseGrandValidator<KnowledgebaseArticleModel>
    {
            ...
            : base(validators)
        {
            ...
            {
                if (await groupService.IsGuest(workContext.CurrentCustomer) && !knowledgebaseSettings.AllowNotRegisteredUsersToLeaveComments)
                {
                    context.AddFailure(translationService.GetResource("Knowledgebase.Article.Comments.OnlyRegisteredUsersLeaveComments"));
                }
