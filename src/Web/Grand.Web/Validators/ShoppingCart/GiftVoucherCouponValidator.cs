    ...
    public class GiftVoucherCouponValidator : BaseGrandValidator<GiftVoucherCouponModel>
    {
            ...
            : base(validators)
        {
            ...
            RuleFor(x => x).CustomAsync(async (x, context, _) =>
            {
                ...
                var giftVoucher = (await mediator.Send(new GetGiftVoucherQuery { Code = x.GiftVoucherCouponCode, IsGiftVoucherActivated = true }, _)).FirstOrDefault();
                var isGiftVoucherValid = giftVoucher != null
                                         && giftVoucher.IsGiftVoucherValid(workContext.WorkingCurrency, workContext.CurrentStore);
