...
public class EmailAccountValidator : BaseGrandValidator<EmailAccountModel>
{
        ...
        : base(validators)
    {
        RuleFor(x => x.Email).NotEmpty()
            .WithMessage(translationService.GetResource("Admin.Configuration.EmailAccounts.Fields.Email"));
        RuleFor(x => x.Email).EmailAddress().WithMessage(translationService.GetResource("Admin.Common.WrongEmail"));
        RuleFor(x => x.DisplayName).NotEmpty()
            .WithMessage(translationService.GetResource("Admin.Configuration.EmailAccounts.Fields.DisplayName"));
    }
}