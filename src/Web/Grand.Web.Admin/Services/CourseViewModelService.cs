...
namespace Grand.Web.Admin.Services
{
        ...
        : ICourseViewModelService
    {
        ...
        public virtual async Task<Course> InsertCourseModel(CourseModel model)
        {
            ...
            //locales
            model.SeName = await course.ValidateSeName(model.SeName, course.Name, true, seoSettings, slugService, languageService);
        ...
        public virtual async Task<Course> UpdateCourseModel(Course course, CourseModel model)
        {
            ...
            course.SeName = model.SeName;
            //locales
            course.Locales = await model.Locales.ToTranslationProperty(course, x => x.Name, seoSettings, slugService, languageService);
            await courseService.Update(course);
            //search engine name
            await slugService.SaveSlug(course, model.SeName, "");
        ...
        public virtual async Task<CourseModel.AssociateProductToCourseModel> PrepareAssociateProductToCourseModel()
        {
            ...
            model.AvailableProductTypes.Insert(0, new SelectListItem { Text = translationService.GetResource("Admin.Common.All"), Value = " " });
            return model;
        }
