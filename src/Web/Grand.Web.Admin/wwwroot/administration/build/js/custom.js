...
function init_sidebar() {
    ...
    $("#sidebar-menu .side-menu > li.has-submenu > .child_menu").each(function () {
        var sub = $(this);
        var li_count = sub.find('li').length;
        if (sub.find('.has-submenu').length > 0) {
            $(sub).addClass("wrapped");
            if (li_count > 20) {
                $(sub).addClass('for-3');
            }
        }
...
function fixSaveButtons() {
    $(".btn-group-devided button[type='submit']").on("click", function () {
        $(this).parent().find('button').addClass("not-clickable").delay(1500).queue(function (next) {
            $(this).parent().find('button').removeClass("not-clickable")
            next();
        });
    });
}
