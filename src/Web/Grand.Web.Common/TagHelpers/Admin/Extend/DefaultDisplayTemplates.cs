...
internal static class DefaultDisplayTemplates
{
    ...
    private static async Task<IHtmlContent> BooleanTemplateCheckbox(bool value)
    {
        ...
        if (value) inputTag.Attributes["checked"] = "checked";
        ...
        try
        {
            ...
            foreach (var item in enumerable)
            {
                ...
                var modelExplorer = new ModelExplorer(
                    metadataProvider,
                    htmlHelper.ViewData.ModelExplorer,
                    itemMetadata,
                    item);
    ...
    {
        if (htmlHelper.ViewData.ModelMetadata.HideSurroundingHtml) return HtmlString.Empty;
        return await StringTemplate(htmlHelper);
    }
    ...
    public static async Task<IHtmlContent> ObjectTemplate(IHtmlHelper htmlHelper)
    {
        ...
        if (modelExplorer.Model == null) return new HtmlString(modelExplorer.Metadata.NullDisplayText);

        if (templateInfo.TemplateDepth > 1)
        {
            var text = modelExplorer.GetSimpleDisplayText();
            if (modelExplorer.Metadata.HtmlEncode) text = htmlHelper.Encode(text);
        ...
        foreach (var propertyExplorer in modelExplorer.Properties)
        {
            ...
            if (!ShouldShow(propertyExplorer, templateInfo)) continue;
