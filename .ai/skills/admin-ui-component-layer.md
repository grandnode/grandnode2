...
- **Add new for a grid:** always in the header of the card that holds the grid, top right, `btn btn-outline-secondary btn-sm` with `bi-plus-lg`. Never under the grid, never in the grid's own toolbar.
...
| `<admin-filters id submit-id expanded>` + `<filters-search>` | `.grand-filters`: one search field, Filters toggle, collapsed two-column panel | `.main-header` + `.drop-filters-container` + ten stacked 3/9 rows |
...
4. Exactly one `btn-primary` per screen or popup. If you find two, one of them is wrong.
