...
| `<admin-filters id submit-id expanded secondary-submit>` + `<filters-search>` | `.grand-filters`: one search field, Filters toggle, collapsed two-column panel; Search is an outline button (`secondary-submit` defaults to true, `false` makes it primary) | `.main-header` + `.drop-filters-container` + ten stacked 3/9 rows |
...
| `<admin-action-menu text>` + `<admin-action text icon url submit name id destructive>` | the `⋮` dropdown, `.grand-action-menu`: 15px entries with an icon, a destructive entry in red under a rule | export/import/copy/delete buttons in the header row |
...
4. Exactly one `btn-primary` per screen or popup. If you find two, one of them is wrong. On a list the primary is Add new; the filter bar's Search is an outline button by default, so a list without Add new (contact forms, orders) has no primary at all, which is right. Never set `secondary-submit="false"` on a screen that already has a primary.
...
- **A filter bar inside a tab.** A tab pane is not a flex column, so `<admin-filters>` followed by a grid card needs the `.grand-tabstrip .grand-filters ~ .grand-card` gap rule (`_components.scss`); the affiliate Orders tab is the example. Search there is an outline button already, so it never competes with the screen's Save.
...
- **The overflow menu is not `.dropdown-menu`.** The component style keys off `.grand-action-menu`, the class `<admin-action-menu>` adds; the rich-text editor on the same screens draws its own `.dropdown-menu` and a rule on that would restyle its colour palette. A hand-written dropdown in a header takes `grand-action-menu` too.
