...
- No panel loads Kendo UI at all; `wwwroot/administration/kendo/` is kept unreferenced for one major release and then deleted. `admin.legacy.js` maps the Kendo jQuery plugins a third-party plugin view may still call onto `GrandAdmin` and warns once per name; never call them from a view in this repository.
