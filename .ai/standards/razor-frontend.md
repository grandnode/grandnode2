...
- Never write a `k-*` class. Kendo is gone; `admin.legacy.css` keeps those names alive for third-party plugin views only.
...
- No panel loads Kendo UI, Bootstrap 4, Font Awesome or simple-line-icons at all; `wwwroot/administration/kendo/` is kept unreferenced for one major release and then deleted. `admin.legacy.js` maps the Kendo jQuery plugins a third-party plugin view may still call onto `GrandAdmin` and warns once per name; never call them from a view in this repository.
